...
gene_counts <- read.table("data/gene_counts.txt", header = TRUE, row.names = 1, sep = "\t")

# Load sample traits metadata
sample_traits <- read.csv("data/SraRunTable.csv", row.names = 1)
...
write.csv(merged_gene_counts, "results/merged_gene_counts.csv")
merged_gene_counts <- read.csv("results/merged_gene_counts.csv", row.names = 1)
...
exprData <- read.csv("results/normalized_counts.csv", row.names = 1)
...
cat("Dimension check - moduleColors:", length(moduleColors), "exprData rows:", nrow(exprData_variable_1), "\n")
...
write.csv(df, "results/Module_Assignments.csv", row.names = FALSE)
cat("Module assignments saved to: results/Module_Assignments.csv\n")
...
pdf("figures/dendrogram_with_module_colors_2.pdf", width = 8, height = 6)  # Adjust width and height as needed
...
png("figures/heatmap_plot.png", width = 1300, height = 1000)



# Open a PDF file with smaller dimensions
pdf("figures/heatmap_plot.pdf", width = 13, height = 12)  # Reduced dimensions
...
png("figures/heatmap_conditions_separate.png", width = 1300, height = 1000)
...
pdf("figures/heatmap_conditions_separate.pdf", width = 13, height = 12)
...
png("figures/heatmap_severity_separate.png", width = 1300, height = 1000)
...
pdf("figures/heatmap_severity_separate.pdf", width = 13, height = 12)
...
png("figures/heatmap_gender_separate.png", width = 1300, height = 1000)
...
pdf("figures/heatmap_gender_separate.pdf", width = 13, height = 12)
...
png("figures/module_membership_distribution.png", width = 800, height = 600)
...
write.csv(data.frame(Gene = hubGenes_yellow), "results/Top10_HubGenes_Yellow.csv", row.names = FALSE)
write.csv(data.frame(Gene = hubGenes_red), "results/Top10_HubGenes_Red.csv", row.names = FALSE)
...
write.csv(data.frame(Gene = top_20_genes), "results/Top20_Overall_HubGenes.csv", row.names = FALSE)
